...
# bug, tgt has to be en, otherwise, METEOR will fail
src=en
tgt=en

cat $1 | trim.sh | txt2xml.sh | wrap-xml.py src $src $tgt >$3.TER-src.xml
if [ "${2:${#2}-1}" == "." ]; then
	wrap-xml.py ref en zh $2 | split-zh-characters.py >$3.TER-ref.xml
elif [ ! -s "$2" ]; then
	wrap-xml.py ref en zh $2. | split-zh-characters.py >$3.TER-ref.xml
else
	cat $2 | split-zh-characters.py | txt2xml.sh | wrap-xml.py ref $src $tgt >$3.TER-ref.xml
fi
cat $3 | split-zh-characters.py | txt2xml.sh | wrap-xml.pl $3.TER-src.xml $tgt >$3.TER-sys.xml
...
# METEOR
echo "METEOR"
java -Xmx2G -jar $HOME/bin/meteor-1.5.jar $3.TER-sys.xml $3.TER-ref.xml -l $tgt -sgml 2>&1 | tee $3.METEOR-score
